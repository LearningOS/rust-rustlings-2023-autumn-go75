...
//
// The externally imported functions are declared in the extern blocks, with a semicolon to mark the end of signature instead of curly braces. Some attributes can be applied to those function declarations to modify the linking behavior, such as #[link_name = ".."] to modify the actual symbol names.
//
...
//
// In this exercise, your task is to make the testcase able to call the `my_demo_function` in module Foo. the `my_demo_function_alias` is an alias for `my_demo_function`, so the two line of code in the testcase should call the same function.
//
